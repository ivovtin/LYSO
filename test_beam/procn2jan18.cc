...
{
        cout<<"Usage: "<<progname<<"\t"<<"wave_0.dat wave_4.dat wave_2.dat test.root 10000 20 0 0 2 2017-12-30_00-49.root"<<endl;
        exit(0);
}
...
//    }
	...
	if(argc < 11 || argc > 11 && atoi(argv[9])!=2 )
	{
         	Usage(progname);
	}
...
//-------- Creation of utime[] from RunDaq ---------------//
    ...
    int ndaqev=daq->GetEntries();                //number events in daq 
    cout<<"DaqEntries:"<<ndaqev<<endl;
    struct serv{
        UInt_t time;
        UInt_t index;
        uint64_t utime;                         //unix time
    } service;
    ...
    {
        daq->GetEntry(i);
        utime[i]=service.utime;                //unix time on event
        if (i==0) {dut[i]=0;}
        else {dut[i]=utime[i]-utime[i-1];}     //unix time between events
        //dtu=dut[i];
